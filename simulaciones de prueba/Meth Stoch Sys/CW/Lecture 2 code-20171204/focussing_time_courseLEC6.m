...
%values
[t_deterministic_first,X_deterministic_first]=ode15s(@(t,X)RHS_focussing(t,X,k1nu,k2,k3,k4onu,k5nu,k6),[0,T_change],[10,100,0]);
...
if AFIXED
    [t_deterministic_second,X_deterministic_second]=ode15s(@(t,X)RHS_focussing(t,X,k1nu,k2,k3,k4onu,k5nu,k6),[T_change,T_final],...
        [k5nu/k6,X_deterministic_first(end,2),X_deterministic_first(end,3)]);
else
    [t_deterministic_second,X_deterministic_second]=ode15s(@(t,X)RHS_focussing(t,X,k1nu,k2,k3,k4onu,k5nu,k6),[T_change,T_final],...
        [X_deterministic_first(end,1),X_deterministic_first(end,2),X_deterministic_first(end,3)]);
end
