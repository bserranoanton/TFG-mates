...
for X_init=0:X_u;
    ...
    for jj=1:M
        ...
        if X<X_u
            %Define loop_condition to be one initially so we enter the
            %loop...
            loop_condition=1;
        else
        ...
        while loop_condition
            ...
            if X>X_u
                ...
                %Check the extra condition in case the path has crossed in
                %between updates
                if exp(-((X_pre-X_u)*(X-X_u))/(k5^2*delta_t))>rand 
                    loop_condition =0;
                end
        ...
        %Increment the time_sum for this value of X_init
