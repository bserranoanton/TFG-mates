...
rec_vector_Y(1,:)=Y_init*ones(1,M);
    ...
    rec_vector_X(2:end,:)=sqrt(2*D*delta_t)*cumsum(randn(num_rec_steps,M),1);
    rec_vector_Y(2:end,:)=sqrt(2*D*delta_t)*cumsum(randn(num_rec_steps,M),1);
    
    
    %Find the mean value of X and Y with time
    mean_X=mean(rec_vector_X,2);
    mean_Y=mean(rec_vector_Y,2);
